/**
 * Queue consumer — triage AND publish. Fully automated, no human in the loop.
 *
...
async function transition(env: Env, id: string, from: string, to: string, detail?: string) {
  await env.DB.batch([
...
async function retrieveCandidates(env: Env, sub: any): Promise<Candidate[]> {
...
function titleFor(sub: any): string {
  const first = (sub.body_sanitized as string).split('\n').find((l) => l.trim()) ?? 'Feedback report';
  const prefix = sub.error_code ? `[${sub.error_code}] ` : '';
  return `${prefix}${first.trim().slice(0, 90)}`;
}

function issueBody(sub: any, env: Env, attachments: StoredAttachment[]): string {
...
async function foldIntoIssue(env: Env, sub: any, issueNumber: number, confidence: number) {
...
export default {
  async queue(batch: MessageBatch<{ submission_id: string }>, env: Env): Promise<void> {
    for (const msg of batch.messages) {
      const id = msg.body.submission_id;
      try {
        const sub = await env.DB.prepare('SELECT * FROM submissions WHERE submission_id = ?').bind(id).first();
        if (!sub) { msg.ack(); continue; }
        if (sub.state === 'published' || sub.state === 'quarantined') { msg.ack(); continue; }

        // --- Idempotency layer 3: has GitHub already got this? -------------
        const already = await markerAlreadyPublished(env.DB, id);
        if (already) {
          await transition(env, id, sub.state as string, 'published', `recovered marker #${already}`);
        ...
        try {
          const r = await classify(sub.body_sanitized as string, candidates, env as any);
          ...
          // duplicates are born. Retry later; the queue holds it.
        ...
        if (verdict.verdict === 'duplicate' && verdict.issue_number && verdict.confidence >= dupGate) {
          ...
          await transition(env, id, sub.state as string, 'published', `folded into #${verdict.issue_number}`);
          msg.ack(); continue;
        }
        ...
        if (verdict.confidence >= reviewGate && verdict.confidence < dupGate && verdict.issue_number) {
          await foldIntoIssue(env, sub, verdict.issue_number, verdict.confidence);
          await transition(env, id, sub.state as string, 'published', `low-confidence fold #${verdict.issue_number}`);
          msg.ack(); continue;
        }
        ...
        if (!decision.allowed) {
          // Backpressure, not data loss. The row stays; the queue re-delivers.
          await transition(env, id, sub.state as string, 'capped', decision.reason);
          console.warn(`capped (${decision.reason}), deferring`, id);
          msg.retry({ delaySeconds: 900 });
          continue;
        }

        const attachments: StoredAttachment[] =
          (JSON.parse((sub.attachment_keys as string) ?? '[]') as string[])
        ...
        await transition(env, id, sub.state as string, 'publishing');
        ...
        if (err instanceof RateLimited) {
          ...
          msg.retry({ delaySeconds: Math.ceil(err.retryAfterMs / 1000) });
          continue;
        }
        console.error('publish failed', id, err);
        msg.retry(); // exhausted → mfv2-dlq
      }
    }
  },
};
