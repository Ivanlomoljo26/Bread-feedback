...
describe('the console shows it as a suggestion', () => {
  it('K16. a classified review renders none of the model\'s output; the classification stays stored', async () => {
    ...
    // Not useful yet (maintainer, 2026-09-15): nothing the model produced is on the page.
    for (const gone of ['What the AI suggests', 'Sends hang at proving.', 'AI suggests', '0.82', 'claude-opus-5', STORE_PROMPT_VERSION]) {
      expect(html, gone).not.toContain(gone);
    }
    const row = await env.DB.prepare('SELECT ai_labels, ai_confidence, ai_model, ai_prompt_version FROM store_reviews WHERE store_review_id = ?')
      .bind(id).first<any>();
    expect(row).toEqual({ ai_labels: '["bug"]', ai_confidence: 0.82, ai_model: 'claude-opus-5', ai_prompt_version: STORE_PROMPT_VERSION });
  });

  it('K17. an unclassified flagged review shows no AI section either', async () => {
    const id = await seedStoreReview({
    ...
    const html = await (await callWorker(new Request(
      `https://mfv2.test/admin/store/${id}`, { headers: await adminHeaders() }))).text();
    expect(html).not.toContain('never sent to the model');
    expect(html).not.toContain('What the AI suggests');
  });
});
